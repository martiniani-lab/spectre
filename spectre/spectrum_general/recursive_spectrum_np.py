...
class recursive_solution_np:
    ...
    def find_solution_recursive(self):
        ...
        for alpha in range(self.n - 1, 0, -1):
            self.P_prime[alpha - 1] = recursive_solution_np.P_prime_func(
                self.J, self.P[alpha], self.P_prime[alpha]
            )
            self.Q_prime[alpha - 1] = recursive_solution_np.P_prime_func(
                self.J, self.Q[alpha], self.Q_prime[alpha]
            )
            self.q[alpha] = recursive_solution_np.q_func(
                self.J, self.Q_prime[alpha - 1], self.Q[alpha], self.n, alpha
            )
            self.P[alpha - 1] = recursive_solution_np.P_func(
                self.q[alpha], self.C, self.P_prime[alpha - 1], self.J, self.P[alpha]
            )
            self.Q[alpha - 1] = recursive_solution_np.P_func(
                self.q[alpha],
                np.eye(self.n),
        ...
        self.q[0] = recursive_solution_np.q_func(
