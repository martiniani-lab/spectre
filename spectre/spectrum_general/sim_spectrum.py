...
class sim_solution:
    ...
    def simulate_sde(self, t, n_points=int(1e5), time=10, dt=1e-4, save=True):
        ...
        """
        key = self.obj.get_instance_variables() + (
            n_points,
            time,
            dt,
            self.noise_type,
        ...
        if key not in self.obj.simulation:
            if hasattr(self.obj, "steady_state"):
                ...
                x0 = self.steady_state(time=time / 10, points=n_points // 10).unsqueeze(
                    0
                )
    ...
    @staticmethod
    def spectrum(
        i=None, j=None, nperseg=1000, sampling_freq=torch.tensor([1e4]), sol_sde=None
    ):
        sol_sde = sol_sde - torch.mean(sol_sde, dim=0)
        sol_i = sol_sde[:, i]

        if j is None:
            freq, S = scipy.signal.welch(
                sol_i.numpy(),
                sampling_freq.numpy(),
                nperseg=nperseg,
                scaling="density",
    ...
    def simulation_spectrum(
        self, i=None, j=None, ndivs=15, n_points=int(1e5), time=10, dt=1e-4
    ):
        """
        ...
        S, f = self.spectrum(
            i=i, j=j, sol_sde=sol_sde, nperseg=nperseg, sampling_freq=sampling_freq
        )
        return S, f

    def simulation_coherence(
        self, i=None, j=None, ndivs=15, n_points=int(1e5), time=10, dt=1e-4
    ):
        """
        ...
        Sxy, f = self.simulation_spectrum(
            i=i, j=j, ndivs=ndivs, n_points=n_points, time=time, dt=dt
        )
        Sxx, _ = self.simulation_spectrum(
            i=i, j=None, ndivs=ndivs, n_points=n_points, time=time, dt=dt
        )
        Syy, _ = self.simulation_spectrum(
            i=j, j=None, ndivs=ndivs, n_points=n_points, time=time, dt=dt
        )
